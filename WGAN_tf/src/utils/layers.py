...
def linear(x, name, n_out, bias=False, activation_fn=None):
    """Dense layer"""
...
def conv2d(x, name, n_in, n_out, k, s, p, bias=False, stddev=0.02, data_format="NCHW"):
    """Conv2D layer"""
    with tf.variable_scope(name):
...
def G_conv2d_block(x, name, f, k, bias=False, bn=False, stddev=0.02, data_format="NCHW", activation_fn=lrelu):
    """G 2D conv block: chain 2 convolutions with"""
    with tf.variable_scope(name):
        ...
        # conv 2d layer
        x = conv2d(x, "conv2d", n_in, n_out, k, stride, padding, stddev=stddev, data_format=data_format, bias=bias)
        if bn:
            x = tf.contrib.layers.batch_norm(x, data_format=data_format, fused=True, scope="BatchNorm")
...
def D_conv2d_block(x, name, f, k, bias=False, bn=False, stddev=0.02, data_format="NCHW", activation_fn=lrelu, downsampling=True):
    """D 2D conv block: chain 2 convolutions with x2 downsampling by default"""
    with tf.variable_scope(name):
